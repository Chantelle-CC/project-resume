  ...
  return (
    <div ref={ref} className="resume-sheet mx-auto px-[16mm] py-[14mm] shadow-lg">
      ...
      <header className="mb-4">
        ...
        {data.title && <p className="mt-0.5 text-[12pt] font-medium text-neutral-700">{data.title}</p>}

        <div className="mt-2.5 flex flex-wrap gap-x-4 gap-y-1 text-[9pt] text-neutral-600">
          {data.email && (
      ...
      {data.summary && (
        <section className="mb-4">
          ...
          <p className="text-[10pt] leading-relaxed text-neutral-800">{data.summary}</p>
        </section>
      )}
            ...
            {data.experience.map((e, i) => (
              <div key={i}>
                <div className="flex items-baseline justify-between">
                  <p className="text-[10.5pt] font-semibold text-neutral-900">
                    {e.role}
                    {e.company && <span className="font-normal text-neutral-600"> · {e.company}</span>}
                  </p>
                  <span className="shrink-0 text-[9pt] text-neutral-500">
                    {[e.start, e.end].filter(Boolean).join(" – ")}
                  </span>
                </div>
                {e.location && <p className="text-[9pt] text-neutral-500">{e.location}</p>}
                <ul className="mt-1 space-y-0.5">
                  {e.highlights.map((h, j) => (
                    <li key={j} className="flex gap-1.5 text-[10pt] leading-relaxed text-neutral-800">
          ...
          <div className="space-y-3">
            {data.projects.map((p, i) => (
              <div key={i}>
                <p className="text-[10.5pt] font-semibold text-neutral-900">
                  {p.name}
                  {p.role && <span className="font-normal text-neutral-600"> · {p.role}</span>}
                </p>
                {p.description && <p className="text-[10pt] leading-relaxed text-neutral-700">{p.description}</p>}
                <ul className="mt-1 space-y-0.5">
            ...
            {data.education.map((ed, i) => (
              <div key={i} className="flex items-baseline justify-between">
                <p className="text-[10pt] text-neutral-800">
                  <span className="font-semibold text-neutral-900">{ed.school}</span>
